...
You will then be able to access it through either `http://domain.tld/calibreweb` or `http://domain.tld:8083`, depending on whether or not you have nginx installed.
...
2. _*_ Change the password of the admin user 
   ...
   - Go to `http://<domain>/calibreweb/me` and change the password there.
...
4. Change the name of the admin user
   ...
   - Go to `http://<domain>/calibreweb/me` and change the details there
      - You might need to fill in an email address for this action to work
3. Link or install necessary binaries
   - Go to `https://<domain>/calibreweb/admin/config` and open the "External binaries" section at the bottom
...
```bash
/etc/systemd/system/calibreweb.service
```
...
<SystemdTabs service="calibreweb"/>
...
### Verbose output
Calibre-web stores its logs in `/opt/calibreweb/calibre-web.log` and optionally `/opt/calibreweb/access.log`.
