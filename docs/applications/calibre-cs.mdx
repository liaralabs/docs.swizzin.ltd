...
After installation, Calibre content server will be accessible on `<domain>/calibre-cs`, or `<domain>:8089`.
...
```bash
/etc/systemd/system/calibre-cs.service
```

import SystemdTabs from '../snippets/systemdtabs.mdx';

<SystemdTabs service="calibre-cs"/>
...
Please edit `/etc/systemd/system/calibre-cs.service`, at the end of the line beginning with `ExecStart=` you should add the path. Make sure that the calibre-cs user 